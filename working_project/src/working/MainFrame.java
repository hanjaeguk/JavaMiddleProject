...
public class MainFrame extends JFrame implements ActionListener {
	...
	JMenuItem salesRegItem, salesStatusItem, 
			stockModifyItem, stockSearchItem, storestockSearchItem,
	...
	public MainFrame(DBcon dbcon) {
		...
		// StoreStockSearch 재고조회
		storestockSearchItem = new JMenuItem("매장 재고조회");
		stockMenu.add(storestockSearchItem);
		storestockSearchItem.addActionListener(this);
		contentPane.add("StoreStockSearch", new StoreStockSearch(myDBcon));
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == salesRegItem) {
		...
		} else if (e.getSource() == storestockSearchItem) {
			card.show(contentPane, "StoreStockSearch");//
			...
			// 로그아웃
			int dialogResult = JOptionPane.showConfirmDialog(this, "로그아웃 하시겠습니까?", "logout", JOptionPane.YES_NO_OPTION,
